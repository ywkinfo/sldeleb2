...
export default function Home() {
  const starter = practiceSets.find((s) => s.id === "reading-tarea-2-1");
  return <>
    <section className="hero"><div className="site-shell hero-grid"><div><p className="section-kicker">Spanish Lab · 스페인어 연구소</p><h1>기출 유형을<br />내 것으로.</h1><p className="lead">흩어진 공식 자료는 한곳에서 찾고, B2 문제는 한국어 해설과 함께 짧게 연습하세요. 로그인도 결제도 필요 없습니다.</p><div className="hero-actions"><a className="button" href={sitePath("/practice")}>짧은 Tarea 연습</a><a className="button secondary" href={sitePath("/materials")}>공식 자료 찾기</a></div></div><div className="hero-note"><strong>{starter?.title ?? "오늘의 짧은 읽기"}</strong><p>{starter ? `약 ${starter.estimatedMin}분 · ${starter.itemIds.length}문항` : "한 Tarea 단위로 부담 없이 시작하세요."}</p><p className="badge warning" style={{ marginTop: ".8rem" }}>공식 기출이 아닌 창작 연습문항</p></div></div></section>
