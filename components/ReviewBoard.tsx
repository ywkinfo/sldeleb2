  ...
  return <>
    ...
    <div className="card flat" style={{ marginTop: "1.2rem" }}><h2>복습 대기열</h2>
      {!review.length ? <div><p className="muted">아직 복습할 문항이 없어요. 연습에서 틀린 문항이나 별표 표시한 과제가 여기에 모입니다.</p><a className="button" href="/practice">연습 시작</a></div> : <div className="review-list">{review.map((attempt) => {
        ...
        return <div className="review-row" key={attempt.itemId}><div><strong>{itemLabel(item)}</strong><div className="muted">{attempt.kind === "open" ? (attempt.completed ? `자기평가 ${attempt.selfScore}/3` : "미완료") : attempt.correct ? "정답" : "오답"} · {attempt.attemptCount}회 시도</div></div><div className="question-actions"><a className="button small" href={`/practice#${item?.id ?? "reading"}`}>다시 연습</a><button className="button secondary small" type="button" onClick={() => remove(attempt.itemId)}>기록 삭제</button></div></div>;
      })}</div>}
    </div>
